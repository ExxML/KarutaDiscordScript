class Config():
    def __init__(self):
        ...
        self.FIGHT_FOR_POG_CARD = False  # (bool) ONLY IF there is exactly one pog card in the drop, one or two other accounts will fight for the pog card (the dropper will always win, of course)- the other two cards in the drop will not be grabbed. Note that if self.ONLY_GRAB_POG_CARDS = False and self.FIGHT_FOR_POG_CARD = True, the fighting will take precedence over grabbing non-pog cards.